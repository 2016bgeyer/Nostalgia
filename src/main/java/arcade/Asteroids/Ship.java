package arcade.Asteroids;
   ...
   public class Ship {
      ...
      //this.x refers to the Ship's x, x refers to the x parameter
         ...
         flameXPts=new int[3];
         flameYPts=new int[3];
         this.shotDelay=5; // # of frames between shots
      ...
      //calculate the polgyon for the ship, then draw it
         for(int i=0;i<4;i++){
            xPts[i]=(int)(origXPts[i]*Math.cos(angle)- /* rotate */ origYPts[i]*Math.sin(angle)+x+.5); //translate and round
            yPts[i]=(int)(origXPts[i]*Math.sin(angle)+ /* rotate */ origYPts[i]*Math.cos(angle)+y+.5); //translate and round
         }
