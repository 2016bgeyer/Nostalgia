   ...
   public class BrickCollisionnew
   {
      ...
      public static void collide2(Bumper[][] myBrickarray, Ball ball)
      {
         for(int k = 0; k<myBrickarray.length; k++)
         {
            for(int r = 0; r<myBrickarray[0].length; r++)
            {
               if(myBrickarray[k][r].inBumper(ball))
               {	   	
               // back the ball up until it is just outside the bumper
                  while(myBrickarray[k][r].inBumper(ball))
                  {
                     ball.setX(ball.getX() - ball.getdx()/10.0);
                     ball.setY(ball.getY() - ball.getdy()/10.0);
                  }
               ...
               // find the point on the edge of the bumper closest to the ball
                  findImpactPoint(myBrickarray[k][r], ball);
